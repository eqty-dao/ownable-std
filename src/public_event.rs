...
/// Canonical contract-facing payload for public events routed into an Ownable.
...
impl Display for PublicEventError {
    fn fmt(&self, f: &mut Formatter<'_>) -> std::fmt::Result {
        match self {
            ...
            PublicEventError::AbiDecode(message) => write!(f, "failed to decode ABI payload: {message}"),
        }
    }
}
...
mod tests {
    ...
    fn public_event_cbor_round_trip_preserves_binary_fields() {
        let event = PublicEvent {
            ...
            transaction_index: 2,
            log_index: 7,
            block_hash: Some(Binary::from(vec![0x11, 0x22])),
        };

        let encoded = crate::abi::cbor_to_vec(&event).expect("serialize public event");
        let decoded: PublicEvent = crate::abi::cbor_from_slice(&encoded).expect("deserialize public event");
    ...
    fn decode_abi_for_checks_event_type_before_decoding() {
        ...
        let err = decode_abi_for::<ConsumeEvent>(&event, "consume").expect_err("must reject wrong event type");
        assert_eq!(
            err,
